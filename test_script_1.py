...
app = Flask( __name__ )
BACKEND_API_URL = "http://localhost:5001/api/process"

# Конфигурация регулирования
MAX_CONCURRENT_REQUESTS = 10  # Максимум одновременных запросов к backend
RATE_LIMIT_WINDOW = 60  # Окно времени для rate limiting в секундах
MAX_REQUESTS_PER_IP = 20  # Максимум запросов от одного IP за окно
...
def process_number():
    """Основной эндпоинт - обработка чисел"""
    # Валидация JSON
    ...
    try:
        # Запускаем обработку в отдельном потоке из пула
...
def health_check():
    ...
    try:
        # Проверяем backend
        backend_response = requests.get( "http://localhost:5001/health", timeout=3 )
...
def get_queue_stats():
    ...
    with ip_lock:
        ...
        for ip, requests_list in ip_requests.items():
            # Фильтруем только запросы в окне времени
